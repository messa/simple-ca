...
def test_create_server_cert():
    ...
    sc = s.create_server_cert(
        ca_cert=ca.cert, ca_key=ca.key, ca_key_password=ca.key_password, cn='localhost', org='ACME', dc='example'
    )
    assert sc.cert
    assert sc.key
...
def test_create_server_cert_with_san_dns():
    ...
    out = subprocess.check_output(['openssl', 'x509', '-noout', '-text'], input=sc.cert.encode()).decode()
